...
export class SkiperTravelsService {
    ...
    async GenerateTravel(inputviaje: SkiperTravelsInput, ip: string): Promise<SkiperTravels> {
        ...
        let searchDriveIfHasTravels = await getConnection().createQueryBuilder(SkiperTravelsTracing, "SkiperTravelsTracing")
            .leftJoinAndSelect("SkiperTravelsTracing.travel", "SkiperTravels")
            .leftJoinAndSelect("SkiperTravelsTracing.travelstatus", "SkiperTravelsStatus")
            .where("SkiperTravelsStatus.codigo <> :codigo ", { codigo: "COBRADO" })
            .andWhere("SkiperTravelsStatus.codigo <> :codigo ", { codigo: "FINALIZADOANTESDETIEMPO" })
            .andWhere("SkiperTravelsStatus.codigo <> :codigo ", { codigo: "CANCELADO" })
            .andWhere("SkiperTravelsStatus.codigo <> :codigo ", { codigo: "RECHAZADO" })
            .andWhere("SkiperTravels.iddriver = :iddriver", { iddriver: inputviaje.iddriver })
            .getOne();
